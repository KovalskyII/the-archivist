...
async def handle_message(message: types.Message):
    ...
    if author_id == KURATOR_ID:
        ...
        #УДАЛЕНИЕ ПЕРКА ЗП - ПОСЛЕ ИСПОЛЬЗОВАНИЯ УДАЛИТЬ КОМАНДУ
        if text_l == "снять артефакт зп":
            bot_id = message.bot.id
            await revoke_perk(bot_id, "зп")
            await message.reply("🧹 Старый артефакт «зп» снят с бота.")
            return
        # -----------------------------------------
...
async def handle_commands_catalog(message: types.Message):
    ...
    curator = [
        ...
        "щедрость обнулить (reply) - обнуляет очки щедрости участника",
        "щедрость обнулить все подтверждаю - обнуляет очки щедрости всего клуба",
        ...
        "жалование база <N> - устанавливает сумму ежедневного жалования для всех",
        "жалование надбавка <N> - устанавливает надбавку для перка надбавка",
    ]
    keyholders = [
