...
async def handle_market_show(message: types.Message):
    ...
    for code, (emoji, title) in PERK_REGISTRY.items():
        ...
        if code == "надбавка":
            ...
            usage = f"<b>Шанс рефанда при проигрыше в играх:</b> {croup}%"
        elif code == "филантроп":
            usage = f"<b>Шанс что ваш дождь окатит еще одного:</b> {phil}%"
        elif code == "везунчик":
            usage = f"<b>Шанс попасть под чужой дождь:</b> {lucky}%"
        elif code == "премия":
            usage = "<b>Модель премии:</b> 20%×2 | 50%×1 | 10%×0.5 | 20%×0"
